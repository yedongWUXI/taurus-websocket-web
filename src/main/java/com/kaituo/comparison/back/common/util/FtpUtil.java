...
//		}
        ...
        for (int i = 0; i < ftpFiles.length; i++) {
            long lRemoteSize = ftpFiles[i].getSize();
            // 本地文件的地址
            File localFileDir = new File(localDir);
            if (!localFileDir.exists()) {
                localFileDir.mkdirs();
            }
            File localFile = new File(localFileDir, ftpFiles[i].getName());
            long localSize = 0;
            ...
            InputStream is = mFTPClient.retrieveFileStream(remotePath + "/" + ftpFiles[i].getName());
            ...
            is.close();
            fos.close();
            isDo = mFTPClient.completePendingCommand();
            if (isDo) {
